...
class SAC(object):
    def __init__(self, obs_shape, action_shape, args, writer):
        ...
        self.shared_cnn = m.SharedCNN(
            obs_shape, args.num_shared_layers, args.num_filters
        ).cuda()
        self.head_cnn = m.HeadCNN(
            self.shared_cnn.out_shape, args.num_head_layers, args.num_filters
        ).cuda()
        actor_encoder = m.Encoder(
            self.shared_cnn,
            self.head_cnn,
            m.RLProjection(self.head_cnn.out_shape, args.projection_dim),
        )
