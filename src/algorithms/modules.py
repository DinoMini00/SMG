...
class AttributionDecoder(nn.Module):
    def __init__(self,action_shape, frame_stack=3, emb_dim=100) -> None:
        super().__init__()
        ...
        self.conv3 = nn.Conv2d(in_channels=64, out_channels=3*frame_stack, kernel_size=3, padding=1)
...
class AttributionPredictor(nn.Module):
    def __init__(self, action_shape, encoder, frame_stack=3):
        super().__init__()
        self.encoder = encoder
        self.decoder = AttributionDecoder(action_shape, frame_stack, encoder.out_dim)
...
class AuxHead(nn.Module):
    def __init__(self, emb_dim, frame_stack=3) -> None:
        super().__init__()
        self.emb_dim = emb_dim
        self.proj = nn.Linear(in_features=emb_dim, out_features=emb_dim*frame_stack)
        self.relu = nn.ReLU()
        
    def forward(self, x):
        x = self.proj(x).view(-1, self.emb_dim)
        x = self.relu(x)
        return x


class AuxEncoder(nn.Module):
    def __init__(self, encoder, aux_head) -> None:
        super().__init__()
        self.encoder = encoder
        self.head = aux_head
...
class ImageDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.interpolate(x, scale_factor=2)
        x = self.relu(x)
        x = 255*self.sigmoid(self.conv3(x))
        return x
...
class ImageMaskDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.interpolate(x, scale_factor=2)
        x = self.relu(x)
        image, mask = self.sigmoid(self.conv3(x))*255, self.sigmoid(self.conv4(x))
        mask = mask.expand(image.shape)
        return image, mask


class CoPredictor(nn.Module):
    def __init__(self, fore_encoder, back_encoder, action_shape, batch_size=128, emb_dim=100, frame_stack=3):
        super().__init__()
        self.fore_encoder = fore_encoder
        self.back_encoder = back_encoder
        self.fore_decoder = ImageMaskDecoder(emb_dim)
        self.back_decoder = ImageDecoder(emb_dim)
        self.inverse_dynamic = InverseDynamics(fore_encoder.encoder, action_shape, 1024)

        self.frame_stack = frame_stack
        self.shape_raw = (batch_size, 9, 84, 84)
        self.shape_modified = (batch_size*3, 3, 84, 84)

    def forward(self, x):
        fore_feat = self.fore_encoder(x)
        back_feat = self.back_encoder(x)
        fore, mask = self.fore_decoder(fore_feat)
        back = self.back_decoder(back_feat)
        # reconstruct
        recon_img = fore * mask + back * (1 - mask)
        shape = fore.shape
        fore = fore.view((int(shape[0]/3), 9, 84, 84))
        back = back.view(fore.shape)
        recon_img = recon_img.view(fore.shape)
        mask = mask.reshape(fore.shape)
        return fore, back, mask, recon_img

    def mask(self, x):
        fore, mask = self.fore_decoder(self.fore_encoder(x))
        mask = mask.reshape(self.shape_raw)
        return mask
    ...
    def back(self, x):
        back = self.back_decoder(self.back_encoder(x))
        shape = back.shape
        back = back.view((int(shape[0]/3), 9, 84, 84))
        return back
    
    def action(self, x, x_next):
        return self.inverse_dynamic(x, x_next)
