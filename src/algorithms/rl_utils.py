...
def compute_attribution_mask(obs_grad, quantile=0.95, frame_stack=3):
    mask = []
    for i in range(0, 3*frame_stack, 3):
...
def make_obs_grid(obs, frame_stack=3, n=4):
    sample = []
    for i in range(n):
        for j in range(0, 3*frame_stack, 3):
            sample.append(obs[i, j : j + 3].unsqueeze(0))
    sample = torch.cat(sample, 0)
    return make_grid(sample, nrow=frame_stack) / 255.0
