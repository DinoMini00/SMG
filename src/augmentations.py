...
def random_overlayV2(x, dataset="places365_standard", alpha=0.5):
    """Randomly overlay an image from Places"""
    global places_iter
    ...
    return ((1 - alpha) * (x / 255.0) + (alpha) * imgs) * 255.0, imgs * 255.0
...
def attribution_augmentation(x, mask, dataset="places365_standard"):
    ...
    s_plus = x * mask
    s_tilde = (((s_plus) / 255.0) + (imgs * (1 - mask))) * 255.0
    s_minus = imgs * 255
    return s_tilde, s_plus, s_minus
...
color_jitter_module = nn.Sequential(ColorJitterLayer(brightness=0.4, 
                                                  contrast=0.4,
                                                  saturation=0.4, 
                                                  hue=0.5, 
                                                  p=1.0, 
...
def color_jitter(x):
    x = x.view(-1, 3, 84, 84)
    x = color_jitter_module(x/255.0)*255.0
    x = x.view(-1, 9, 84, 84)
    return x
